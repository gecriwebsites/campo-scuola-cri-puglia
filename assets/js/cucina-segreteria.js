(() => {
  ...
  function isKitchenStation(value) { return /^Cucina\s+[123]$/i.test(String(value || '').trim()); }
  ...
  function applyMealCounter(tipo, row) {
    ...
    const ids = map[tipo]; if (!ids) return;
  ...
  async function searchPeople() {
    ...
    if (query.length < 2) { els.searchResults.innerHTML = ''; return; }
  ...
  function renderPerson(data) {
    ...
    window.dispatchEvent(new CustomEvent('campo:kitchen-person-opened', { detail:{ personId:currentPersonId } }));
  }
  ...
  async function consumeMeal(tipo) {
    ...
    const { data, error } = await client.rpc('cucina_consuma_ticket_persona', { p_persona_id:currentPersonId, p_tipo:tipo, p_data:els.date.value || null, p_postazione:station });
    if (error) showToast(`Consumo non registrato: ${error.message}`, 'error');
    else if (data?.status === 'consumato') showToast(`${mealLabels[tipo]} registrato · ${station}.`, 'success');
    ...
    window.dispatchEvent(new CustomEvent('campo:kitchen-meal-consumed', { detail:{ tipo, status:data?.status || (error ? 'error' : '') } }));
  }

  function scheduleReload() {
    clearTimeout(reloadTimer);
    reloadTimer = setTimeout(async () => { await loadDashboard(); if (currentPersonId) await loadPerson(currentPersonId); }, 180);
  }
  ...
  function bindEvents() {
    ...
    els.scan.addEventListener('click', async () => {
      try { await window.CampoQrScanner.open({ title:'Scansiona QR per i pasti', subtitle:'Inquadra il QR personale per visualizzare i ticket del giorno.', onScan:readQr }); }
  ...
  async function waitForStation() {
    for (let i = 0; i < 180; i += 1) {
      if (isKitchenStation(getStation())) return true;
  ...
  async function init() {
    ...
    if (profile?.ruolo !== 'cucina') return;
    els.workspace.hidden = false;
    const title = $('reservedAreaTitle'); if (title) title.textContent = 'Area Cucina';
    els.date.value = todayRome();
    bindEvents();
    const stationReady = await waitForStation();
    if (!stationReady) { showToast('Seleziona Cucina 1, Cucina 2 o Cucina 3.', 'error'); return; }
    connectRealtime();
    await loadDashboard();
  }
...
(() => {
  ...
  function injectUi() {
    ...
    fragment.innerHTML = `
      ...
      <div class="dietary-editor" id="personDietaryEditor">
        <label class="switch-row"><input id="personDietaryPresent" type="checkbox"><span><b>Allergie / intolleranze alimentari</b><small>Segnalazione operativa condivisa con la Cucina</small></span></label>
        <label id="personDietaryDetailsWrap" class="dietary-details" hidden>Dettaglio dell'esigenza<textarea id="personDietaryDescription" class="field-textarea" rows="3" placeholder="Es. celiachia, allergia alla frutta a guscio, intolleranza al lattosio…"></textarea></label>
        <div id="personDietarySaveState" class="dietary-save-state" aria-live="polite"></div>
      </div>`;
    iceGrid.after(...fragment.childNodes);
  }

  function setState(message = '', type = '') {
    const el = $('personDietarySaveState'); if (!el) return;
    el.textContent = message; el.className = `dietary-save-state${type ? ` ${type}` : ''}`;
  }

  function refreshDetailsVisibility() {
    const present = $('personDietaryPresent'), wrap = $('personDietaryDetailsWrap'), description = $('personDietaryDescription');
    if (!present || !wrap || !description) return;
    wrap.hidden = !present.checked; description.disabled = !present.checked; if (!present.checked) description.value = '';
  }

  function currentPersonId() { return String($('personId')?.value || '').trim() || null; }

  async function loadDietaryForCurrentPerson() {
    const personId = currentPersonId(); if (!personId || $('personModal')?.hidden) return;
    loadedPersonId = personId; rowExists = false; setState('Caricamento…');
    const { data, error } = await client.from('esigenze_alimentari').select('persona_id,presente,descrizione').eq('persona_id', personId).maybeSingle();
    if (loadedPersonId !== personId) return;
    if (error) { setState(`Impossibile leggere la segnalazione: ${error.message}`, 'error'); return; }
    rowExists = !!data; $('personDietaryPresent').checked = !!data?.presente; $('personDietaryDescription').value = data?.descrizione || '';
  ...
  async function saveDietary() {
    ...
    const personId = currentPersonId(); if (!personId || personId !== loadedPersonId || $('personModal')?.hidden) return;
    const present = !!$('personDietaryPresent')?.checked;
    const description = present ? (String($('personDietaryDescription')?.value || '').trim() || null) : null;
    if (!rowExists && !present) { setState('Nessuna esigenza alimentare segnalata'); return; }
    localSaveInProgress = true; setState('Salvataggio…', 'saving');
    const result = rowExists
      ? await client.from('esigenze_alimentari').update({ presente:present, descrizione:description }).eq('persona_id', personId)
      : await client.from('esigenze_alimentari').insert({ persona_id:personId, presente:present, descrizione:description });
    localSaveInProgress = false;
    if (result.error) { setState(`Salvataggio non riuscito: ${result.error.message}`, 'error'); return; }
    rowExists = true; setState(present ? 'Salvato · disponibile anche alla Cucina' : 'Salvato · nessuna esigenza alimentare', 'saved');
  }

  function scheduleSave() { clearTimeout(saveTimer); saveTimer = setTimeout(saveDietary, 650); }

  function bindUi() {
    $('personDietaryPresent')?.addEventListener('change', async () => { refreshDetailsVisibility(); await saveDietary(); });
  ...
  function connectRealtime() {
    ...
    realtimeChannel.on('postgres_changes', { event:'*', schema:'public', table:'esigenze_alimentari' }, payload => {
      const personId = payload.new?.persona_id || payload.old?.persona_id;
      if (!localSaveInProgress && personId && personId === loadedPersonId && !$('personModal')?.hidden) loadDietaryForCurrentPerson();
    }).subscribe();
  }
  ...
  async function init() {
    ...
    injectStyles(); injectUi(); bindUi(); connectRealtime(); if (!$('personModal')?.hidden) await loadDietaryForCurrentPerson();
  }
